...
export const findSalesByYear: IFindSaleServerAction = async (args) => {
  ...
  const end = new UTCDate(year, DEC, 31);
...
export const findSalesByMonth: IFindSaleServerAction = async (args) => {
  ...
  const start = startOfMonth(new UTCDate(year, month));
  const end = endOfMonth(new UTCDate(year, month));
...
export const findSalesByWeek: IFindSaleServerAction = async (args) => {
  ...
  if (
    !args ||
    typeof args.year === "undefined" ||
    typeof args.month === "undefined" ||
    typeof args.week === "undefined"
  ) {
    return NextResponse.json(
  ...
  eachDayOfInterval({ start: startDateOfMonth, end: endDateOfMonth }).forEach(
    (date) => {
      const weekOfMonth = getWeekOfMonth(date, { weekStartsOn: 1 });
      if (!weeksMap.has(weekOfMonth)) {
        weeksMap.set(weekOfMonth, date);
      }
    }
  );

  const firstDayOfWeek = weeksMap.get(week);
  if (!firstDayOfWeek) {
    return NextResponse.json(
